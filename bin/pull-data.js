...
function processData() {
	...
	secondarySeries[rawData.dateLaZiData.currentDayStats.parsedOnString] =
		rawData.dateLaZiData.currentDayStats;
	...
	for (const d of mainSeries) {
		...
		let vaccineData = vaccineSeries.filter(function (element) {
			return element.data_date === d.reporting_date;
		})[0];
		...
		if (!vaccineData) {
			...
			noNewVaccineDosesAdministered =
				vaccineData.mega_total_pfizer +
				vaccineData.mega_total_moderna +
				vaccineData.mega_total_astrazeneca +
				vaccineData.mega_total_johnson;
